#!/bin/bash
# iptables logging setup and collection for BlaqLiq detection

CHAIN="BLAQLIQ-DETECT"
LOG_FILE="/var/log/blaqliq-iptables.log"
...
case "$1" in
    setup)
        ...
        # Log with BLAQLIQ-DETECT prefix
        iptables -A "$CHAIN" -j LOG --log-prefix "BLAQLIQ-DETECT " --log-level 4 2>/dev/null || true
    ...
    collect)
        # Read kernel log for BLAQLIQ-DETECT entries
        if [ -f /var/log/kern.log ]; then
            # Get new entries since last check
            grep "BLAQLIQ-DETECT" /var/log/kern.log 2>/dev/null | tail -50 | while read -r line; do
        ...
        dmesg 2>/dev/null | grep "BLAQLIQ-DETECT" | tail -20 | while read -r line; do
            SRC=$(echo "$line" | grep -oP 'SRC=\K\S+' || echo "unknown")
            DPT=$(echo "$line" | grep -oP 'DPT=\K\d+' || echo "0")
            printf '{"type":"iptables_log","src_ip":"%s","dst_port":%s,"raw":"%s","ts":"%s"}\n' \
                "$SRC" "$DPT" "${line//\"/\'}" "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
